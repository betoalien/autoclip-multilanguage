    ...
    def call(self, prompt: str, input_data: Any = None, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            if input_data is not None:
                if isinstance(input_data, (dict, list, tuple)):
                    data_str = json.dumps(input_data, ensure_ascii=False, indent=2, default=str)
                else:
                    data_str = str(input_data)
                messages = [
                    {"role": "system", "content": prompt},
                    {"role": "user", "content": f"请分析以下输入内容，并严格按要求直接输出有效的 JSON 数据，不要包含任何对话前缀或额外文字：\n\n{data_str}"}
                ]
            else:
            ...
            try:
                ...
                logger.warning(f"系统提示词调用可能不受支持 ({system_err})，尝试回退为单条用户提示")
                response = self.client.chat.completions.create(
                    model=self.model_name,
                    messages=[{"role": "user", "content": full_input}],
                    **kwargs
