    ...
    try:
        ...
        try:
            ...
            if user_lang.startswith("es"):
                project_desc = f"Descargado de Bilibili: {video_info.title}"
            elif user_lang.startswith("en"):
                project_desc = f"Downloaded from Bilibili: {video_info.title}"
            else:
                project_desc = f"从B站下载: {video_info.title}"
...
async def process_download_task(task_id: str, request: BilibiliDownloadRequest, project_id: str):
    ...
    try:
        ...
        # 更新项目进度
        msg_down = "Descargando video..." if is_es else ("Downloading video..." if is_en else "正在下载视频...")
        await update_project_download_progress(project_id, 30.0, msg_down)
        ...
        # 更新项目进度
        msg_sub = "Descarga de video completada, procesando subtítulos..." if is_es else ("Video download complete, processing subtitles..." if is_en else "视频下载完成，正在处理字幕...")
        await update_project_download_progress(project_id, 60.0, msg_sub)
        ...
        try:
            ...
            user_lang = (getattr(request, 'language', None) or 'es').lower()
            if user_lang.startswith("es"):
                project.description = f"Descargado de Bilibili: {video_info.title}"
            elif user_lang.startswith("en"):
            ...
            if not srt_file_path.exists():
                ...
                if not project.processing_config:
                    project.processing_config = {}
                fail_reason = "No se encontraron subtítulos y la generación con Whisper falló" if is_es else ("Subtitle file not found and Whisper generation failed" if is_en else "字幕文件不存在且Whisper生成失败")
                project.processing_config["error_message"] = fail_reason
                db.commit()
                
                # 更新任务状态为失败
                download_tasks[task_id].status = "failed"
                download_tasks[task_id].error_message = fail_reason
                ...
                # 更新项目下载进度为失败
                fail_prog = "Descarga fallida: Archivo de subtítulos no encontrado" if is_es else ("Download failed: Subtitle file not found" if is_en else "下载失败：字幕文件不存在")
                await update_project_download_progress(project_id, 0.0, fail_prog)
