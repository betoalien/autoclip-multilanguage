...
async def upload_files(
    ...
    try:
        ...
        # 创建项目数据
        user_lang = (language or "es").lower()
        subtitle_info = srt_file.filename if srt_file else ("Whisper auto" if user_lang.startswith("en") else ("Subtítulos Whisper" if user_lang.startswith("es") else "Whisper自动生成"))
...
async def start_processing(
    ...
    try:
        ...
        # 提交Celery任务
        celery_task = process_video_pipeline.apply_async(
            args=[project_id, str(video_path), str(srt_path) if srt_path else None],
            queue='processing'
        )
...
async def retry_processing(
    ...
    try:
        ...
        if not video_path.exists():
            ...
            # 检查项目元数据或处理配置中是否有源URL
            source_url = None
            if hasattr(project, 'project_metadata') and project.project_metadata:
                source_url = project.project_metadata.get('source_url')
            if not source_url and hasattr(project, 'processing_config') and project.processing_config:
                source_url = (
                    project.processing_config.get('youtube_info', {}).get('url') or
                    project.processing_config.get('bilibili_info', {}).get('url') or
                    project.processing_config.get('source_url')
                )
            if not source_url and hasattr(project, 'settings') and project.settings:
                source_url = (
                    project.settings.get('youtube_info', {}).get('url') or
                    project.settings.get('bilibili_info', {}).get('url') or
                    project.settings.get('source_url')
                )
            ...
            if source_url:
                ...
                if 'bilibili.com' in source_url:
                    ...
                    download_request = BilibiliDownloadRequest(
                        ...
                        video_category=(project.project_metadata.get('category', 'general') if project.project_metadata else 'general')
                    )
                    ...
                    # YouTube视频重新下载
                    from .youtube import process_youtube_download_task, YouTubeDownloadRequest, download_tasks, YouTubeDownloadTask
                    import uuid
                    from datetime import datetime
                    ...
                    # 创建并记录任务对象，避免 process_youtube_download_task 产生 KeyError
        ...
        celery_task = process_video_pipeline.apply_async(
            args=[project_id, str(video_path), srt_path_str],
            queue='processing'
        )
