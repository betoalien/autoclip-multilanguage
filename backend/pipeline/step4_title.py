...
class TitleGenerator:
    ...
    def generate_titles(self, high_score_clips: List[Dict]) -> List[Dict]:
        ...
        logger.info(f"开始为 {len(high_score_clips)} 个高分片段进行批量标题生成... (语言: {self.language})")
        ...
        for chunk_index, chunk_clips in clips_by_chunk.items():
            ...
            try:
                ...
                if self.language.startswith("es"):
                    prompt += "\n\nOBLIGATORIO: Genera todos los títulos de los clips obligatoriamente en ESPAÑOL (Spanish). Deben ser títulos atractivos y virales para redes sociales (Reels/Shorts/TikTok).\n"
                elif self.language.startswith("en"):
                    prompt += "\n\nIMPORTANT: Generate all clip titles strictly in ENGLISH.\n"
                
                raw_response = self.llm_client.call_with_retry(prompt, input_for_llm)
                ...
                if isinstance(titles_map, list):
                    # 如果返回的是数组格式 [{"id": "1", "title": "..."}]，自动转换为字典
                    new_map = {}
                    for item in titles_map:
                        if isinstance(item, dict):
                            cid = str(item.get('id', ''))
                            title = item.get('title') or item.get('generated_title') or item.get('name')
                            if cid and title:
                                new_map[cid] = title
                    titles_map = new_map
                
                if not isinstance(titles_map, dict):
                    logger.warning(f"  > LLM返回的标题不是字典或有效列表: {titles_map}，使用原始outline作为标题")
                    titles_map = {}

                for clip in chunk_clips:
                    clip_id = clip.get('id')
                    generated_title = titles_map.get(clip_id) or titles_map.get(str(clip_id))
...
def run_step4_title(high_score_clips_path: Path, output_path: Optional[Path] = None, metadata_dir: Optional[str] = None, prompt_files: Dict = None, language: str = "es") -> List[Dict]:
    """
    运行Step 4: 标题生成
    ...
    if metadata_dir is None:
        metadata_dir = METADATA_DIR
    title_generator = TitleGenerator(metadata_dir=Path(metadata_dir), prompt_files=prompt_files, language=language)
