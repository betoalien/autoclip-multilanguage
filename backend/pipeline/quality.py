...
class DurationProfile:
    ...
    def prompt_hint(self, language: str = "es") -> str:
        """追加到 step1 / step2 提示词末尾，覆盖提示词里写死的时长规则。"""
        lo, hi = self.target_clip_sec
        n_lo, n_hi = self.topics_hint
        if language.startswith("es"):
            total_str = f"{int(self.total_sec // 60)} min {int(self.total_sec % 60)} s"
            return (
                "\n\n---\n\n## PARÁMETROS DE LA TAREA (PRIORIDAD MÁXIMA SOBRE CUALQUIER REGLA ANTERIOR)\n"
                f"- Duración total del video: {total_str} (Nivel {self.tier})\n"
                f"- Se sugiere extraer entre {n_lo} y {n_hi} temas para todo el video; sin solapamiento entre temas.\n"
                f"- Duración objetivo de cada clip: {_fmt_dur_es(lo)} a {_fmt_dur_es(hi)}, mínimo {_fmt_dur_es(self.min_clip_sec)}, MÁXIMO {_fmt_dur_es(self.max_clip_sec)} (formato Reels / Shorts de ~90 segundos).\n"
                "- OBLIGATORIO: Genera todo el contenido (títulos, esquemas, resúmenes) estrictamente en ESPAÑOL.\n"
                "- Las marcas de tiempo de inicio y fin deben corresponder exactamente a los límites de los subtítulos provistos.\n"
            )
        elif language.startswith("en"):
            total_str = f"{int(self.total_sec // 60)} min {int(self.total_sec % 60)} s"
            return (
                "\n\n---\n\n## TASK PARAMETERS (OVERRULES ALL PREVIOUS DURATION RULES)\n"
                f"- Total video duration: {total_str} (Tier {self.tier})\n"
                f"- Recommended topics count: {n_lo}–{n_hi} non-overlapping topics.\n"
                f"- Target clip length: {int(lo)}–{int(hi)}s, minimum {int(self.min_clip_sec)}s, MAXIMUM {int(self.max_clip_sec)}s (reels/shorts format of ~90s).\n"
                "- Output all text strictly in ENGLISH.\n"
                "- Timestamps must align strictly with subtitle cue boundaries.\n"
            )
        else:
            ...
            return (
                ...
                f"- 每个片段目标时长 {_fmt_dur(lo)}–{_fmt_dur(hi)}，最短不少于 {_fmt_dur(self.min_clip_sec)}，最长不超过 {_fmt_dur(self.max_clip_sec)}（短视频/Reels 90秒规格）\n"
...
def _fmt_dur_es(sec: float) -> str:
    sec = int(round(sec))
    if sec < 60:
        return f"{sec} segundos"
    if sec % 60 == 0:
        return f"{sec // 60} minutos"
    return f"{sec // 60} min {sec % 60} s"


def profile_for(total_sec: float) -> DurationProfile:
    """按总时长分档。目标切片时长适配短视频 Reels/Shorts 约 90 秒。"""
    total_sec = max(0.0, float(total_sec))
    if total_sec < 8 * 60:
        return DurationProfile(
            tier="short", total_sec=total_sec,
            min_clip_sec=30, target_clip_sec=(45, 90), max_clip_sec=95,
            topics_hint=(2, 5), min_keep=2, max_clips=5,
        )
    if total_sec < 30 * 60:
        return DurationProfile(
            tier="medium", total_sec=total_sec,
            min_clip_sec=45, target_clip_sec=(60, 90), max_clip_sec=95,
            topics_hint=(3, 8), min_keep=3, max_clips=8,
        )
    # 长视频：强制切片时长在 60-90 秒范围（最长不超过 95 秒，避免出现 6 分钟过长视频）
    hours = total_sec / 3600
    return DurationProfile(
        tier="long", total_sec=total_sec,
        min_clip_sec=45, target_clip_sec=(60, 90), max_clip_sec=95,
        topics_hint=(max(6, int(6 * hours)), max(10, int(12 * hours))),
        min_keep=3, max_clips=max(8, int(12 * hours)),
    )
