...
class ClipScorer:
    ...
    def __init__(self, prompt_files: Dict = None, metadata_dir: Path = None, language: str = "es"):
    ...
    def _get_llm_evaluation(self, clips: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            if self.language.startswith("es"):
                prompt += "\n\nOBLIGATORIO: Redacta la razón de recomendación (recommend_reason) obligatoriamente en ESPAÑOL.\n"
            elif self.language.startswith("en"):
                prompt += "\n\nIMPORTANT: Write the recommendation reason (recommend_reason) strictly in ENGLISH.\n"
...
def run_step3_scoring(timeline_path: Path, metadata_dir: Path = None, output_path: Optional[Path] = None, prompt_files: Dict = None, language: str = "es") -> List[Dict]:
    """
    运行Step 3: 内容评分与筛选
    ...
    scorer = ClipScorer(prompt_files=prompt_files, metadata_dir=metadata_dir, language=language)
