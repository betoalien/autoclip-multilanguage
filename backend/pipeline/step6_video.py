...
class VideoGenerator:
    ...
    def generate_clips(self, clips_with_titles: List[Dict], input_video: Path, subtitle_path: Optional[Path] = None) -> List[Path]:
        """
        生成切片视频
        ...
        # 批量生成切片（若有字幕则自动烧录大黄字白边字幕）
        successful_clips = self.video_processor.batch_extract_clips(input_video, clips_data, subtitle_path=subtitle_path)
...
def run_step6_video(clips_with_titles_path: Path, collections_path: Path, 
                   ...
                   metadata_dir: Optional[str] = None, subtitle_path: Optional[Path] = None) -> Dict:
    """
    运行Step 6: 视频切割
    ...
    Args:
        ...
        input_video: 输入视频路径
        output_dir: 输出目录
        subtitle_path: 可选的字幕文件路径（将自动切分并烧录大黄字白边字幕）
    ...
    # 生成切片视频（自动烧录字幕）
    successful_clips = generator.generate_clips(clips_with_titles, input_video, subtitle_path=subtitle_path)
