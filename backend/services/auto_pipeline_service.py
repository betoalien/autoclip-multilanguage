    ...
    async def auto_start_pipeline(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                elif (project_dir / "raw" / "input.mp4").exists():
                    input_video_path = str(project_dir / "raw" / "input.mp4")
                else:
                    raw_dir = project_dir / "raw"
                    v_files = list(raw_dir.glob("*.mp4")) or list(raw_dir.glob("*.mkv")) or list(raw_dir.glob("*.webm")) if raw_dir.exists() else []
                    if v_files:
                        input_video_path = str(v_files[0])
                
                if not input_video_path or not Path(input_video_path).exists():
                    raise ValueError(f"视频文件不存在: {input_video_path or project_dir}")

                # 确定字幕文件路径
                input_srt_path = ""
                if srt_file and Path(srt_file).exists():
                    input_srt_path = str(srt_file)
                elif project.subtitle_path and Path(project.subtitle_path).exists():
                    input_srt_path = str(project.subtitle_path)
                elif (project_dir / "raw" / "input.srt").exists():
                    input_srt_path = str(project_dir / "raw" / "input.srt")
                else:
                    raw_dir = project_dir / "raw"
                    s_files = list(raw_dir.glob("*.srt")) if raw_dir.exists() else []
                    if not s_files:
                        s_files = list(project_dir.glob("*.srt"))
                    if s_files:
                        input_srt_path = str(s_files[0])
                
                logger.info(f"视频文件: {input_video_path}")
                logger.info(f"字幕文件: {input_srt_path if (input_srt_path and Path(input_srt_path).exists()) else '不存在（将调用Whisper）'}")
