...
const ProjectDetailPage: React.FC = () => {
  ...
  const handleStartProcessing = async () => {
    ...
    try {
      ...
      message.error(t('projectDetail.startProcessingFailed'))
    }
  }
  ...
  const handleRetryProcessing = async () => {
    ...
    try {
      ...
      message.success(t('projectDetail.retryStarted'))
      loadProcessingStatus()
      await loadProject()
    } catch (err) {
  ...
  const handleCreateCollection = async (title: string, summary: string, clipIds: string[]) => {
    ...
    try {
      ...
      message.error(t('projectDetail.collectionCreateFailed'))
    }
  }
  ...
  const handleRemoveClipFromCollection = async (collectionId: string, clipId: string): Promise<void> => {
    ...
    try {
      ...
      message.error(t('projectDetail.clipRemoveFailed'))
    }
  }
  ...
  const handleDeleteCollection = async (collectionId: string) => {
    ...
    try {
      ...
      message.error(t('projectDetail.collectionDeleteFailed'))
    }
  }
  ...
  const handleReorderCollectionClips = async (collectionId: string, newClipIds: string[]): Promise<void> => {
    ...
    try {
      ...
      message.success(t('projectDetail.collectionReordered'))
    } catch (err) {
      console.error('Failed to reorder collection clips:', err)
      message.error(t('projectDetail.collectionReorderFailed'))
    }
  }
  ...
  const handleAddClipToCollection = async (collectionId: string, clipIds: string[]): Promise<void> => {
    ...
    try {
      ...
      message.error(t('projectDetail.clipAddFailed'))
    }
  }
    ...
    return (
      <div className="ac-page">
        <div className="ac-empty">
          <b>{t('projectDetail.loadFailedTitle')}</b>
    ...
    <div className="ac-page">
      ...
      <header>
        <button className="ac-back" onClick={() => navigate('/')}>
          <Icon.Back /> {t('projectDetail.project')}
        </button>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24 }}>
          <div style={{ minWidth: 0 }}>
            ...
            <div className="ac-meta">
              {isCompleted ? (
                ...
                <span>{currentProject.status === 'pending' ? t('projectDetail.statusPending') : isFailed ? t('projectDetail.statusFailed') : t('projectDetail.statusProcessing')}</span>
              )}
          ...
          {isFailed && (
            <div style={{ display: 'flex', gap: 8, flex: '0 0 auto' }}>
              <Btn onClick={() => setFeedbackOpen(true)}>{t('projectDetail.reportIssue')}</Btn>
              <Btn variant="cta" onClick={handleRetryProcessing} loading={statusLoading}>{t('common.retry')}</Btn>
            </div>
          )}
        </div>
      </header>
      ...
      {isCompleted ? (
        <>
          {/* 合集 */}
          <Section
            title={t('projectDetail.collections')}
            count={collections.length}
            description={collections.length > 0 ? t('projectDetail.collectionsDescription') : t('projectDetail.collectionsDescriptionEmpty')}
            right={
          ...
          >
            {collections.length > 0 ? (
              ...
              <div className="ac-empty">
                ...
                {t('projectDetail.noCollectionsHint')}
              </div>
            )}
          </Section>
          ...
          {/* 切片 */}
          <Section
            title={t('projectDetail.clips')}
            count={clips.length}
            right={
              <Segmented
                size="sm"
                ariaLabel={t('projectDetail.sortAriaLabel')}
                value={sortBy}
                onChange={setSortBy}
          ...
          >
            {clips.length > 0 ? (
              ...
              <div className="ac-empty">
                ...
                {t('projectDetail.noClipsHint')}
                <div style={{ marginTop: 12 }}>
                  <Btn variant="text" size="sm" onClick={() => setFeedbackOpen(true)}>{t('projectDetail.tellUs')}</Btn>
                </div>
              </div>
            )}
          </Section>
        ...
        <div className="ac-empty" style={{ marginTop: 32 }}>
          ...
          {currentProject.error_message ? (
            ...
            t('projectDetail.processingFailedHint')
          )}
        </div>
      ) : (
        <div style={{ marginTop: 32 }}>
          ...
          <div className="ac-empty" style={{ marginTop: 24 }}>
            ...
            {t('projectDetail.stillProcessingHint')}
          </div>
        </div>
      )}
