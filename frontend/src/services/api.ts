...
// Attach current app language to all backend requests
api.interceptors.request.use((config) => {
  const lang = i18n.language || (typeof window !== 'undefined' ? localStorage.getItem('autoclip-language') : null) || 'es'
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 429) {
      const message = error.response?.data?.detail || i18n.t('api.rateLimited')
      error.userMessage = message
    }
    else if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
      error.userMessage = i18n.t('api.requestTimeout')
    }
    else if (error.code === 'NETWORK_ERROR' || !error.response) {
      error.userMessage = i18n.t('api.networkError')
    }
    else if (error.response?.status >= 500) {
      error.userMessage = i18n.t('api.serverError')
    }
...
export const projectApi = {
  ...
  uploadFiles: async (data: UploadFilesRequest): Promise<Project> => {
    ...
    formData.append('language', i18n.language || 'es')
