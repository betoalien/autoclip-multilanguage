...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      message.success(t('fileUpload.projectCreatedSuccess'))
      ...
      let errorMessage = t('fileUpload.uploadFailedRetry')
      let errorType = 'error'
      ...
      if (error.response?.status === 413) {
        ...
        if (error.response?.data?.detail) {
          ...
          errorMessage = t('fileUpload.fileContentIssue')
        }
      } else if (error.response?.status === 500) {
        errorMessage = t('fileUpload.serverProcessingError')
      ...
      if (error.code === 'ECONNABORTED' || error.response?.status >= 500) {
        message.info(t('fileUpload.contactSupportIfPersists'), 5)
      }
    } finally {
      setUploading(false)
    }
    ...
    <div style={{
      ...
      >
        ...
        <div>
          <Text strong style={{
            ...
            {isDragActive ? t('fileUpload.dropToImport') : t('fileUpload.clickOrDragToImport')}
          </Text>
          <Text style={{ color: 'var(--ac-sub)', fontSize: '14px', lineHeight: '1.5' }}>
            {t('fileUpload.supportedFormats')} <Text style={{ color: '#52c41a', fontWeight: 600 }}>{t('fileUpload.optionalSubtitleOrAi')}</Text>
          </Text>
        </div>
      </div>
          ...
          {files.video && !files.srt && (
            <div style={{
              ...
              <Text style={{ color: '#52c41a', fontSize: '14px', fontWeight: 500 }}>
                {t('fileUpload.willUseAiSubtitles')}
              </Text>
            </div>
          )}
        </div>
      ...
      {uploading && (
        <div style={{
          ...
          <Text style={{ color: 'var(--ac-sub)', fontSize: '13px', marginTop: '8px', display: 'block', textAlign: 'center' }}>
            {t('fileUpload.importingPleaseWait')}
          </Text>
        </div>
      )}
      ...
      {files.video && (
        <div style={{ textAlign: 'center', marginTop: '8px' }}>
          ...
          >
            {uploading ? t('fileUpload.importing') : t('fileUpload.startImportAndProcess')}
          </Button>
        </div>
      )}
    </div>
