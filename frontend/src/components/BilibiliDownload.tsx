...
import { getLocalizedProgressMessage } from '../stores/useSimpleProgressStore'
...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      try {
        ...
        message.error(t('bilibili.download.loadCategoriesFailed'))
      } finally {
  ...
  const parseVideoInfo = async () => {
    if (!url.trim()) {
      setError(t('bilibili.download.errorEnterValidUrl'))
      return
    }

    const videoType = getVideoType(url.trim())
    if (!videoType) {
      setError(t('bilibili.download.errorEnterValidBiliOrYoutubeUrl'))
      return
    }
    ...
    const interval = window.setInterval(async () => {
      try {
        ...
        if (task.status === 'completed') {
          ...
          message.success(t('bilibili.download.downloadComplete'))
          ...
          setPollingInterval(null)
          setDownloading(false)
          message.error(t('bilibili.download.downloadFailed', { reason: task.error_message || t('bilibili.download.unknownError') }))
          resetForm()
        }
      } catch (error) {
  ...
  const handleDownload = async () => {
    if (!url.trim()) {
      message.error(t('bilibili.download.errorEnterUrl'))
      return
    }

    const videoType = getVideoType(url.trim())
    if (!videoType) {
      message.error(t('bilibili.download.errorEnterValidBiliOrYoutubeUrl'))
      return
    }
    ...
    try {
      ...
      if (response.project_id) {
        addProject({
          ...
          name: projectName.trim() || (videoInfo?.title ?? t('bilibili.download.newProjectDefaultName')),
          status: 'pending',
          source_url: url.trim(),
        ...
        // 显示统一的成功提示
        const platformName = videoType === 'bilibili' ? t('bilibili.download.platformBilibili') : 'YouTube'
        message.success(t('bilibili.download.projectCreatedSuccess', { platform: platformName }))
      ...
      setDownloading(false)
      const errorMessage = error.response?.data?.detail || error.message || t('bilibili.download.createTaskFailed')
  ...
  const stopDownload = () => {
    ...
    setDownloading(false)
    setCurrentTask(null)
    message.info(t('bilibili.download.stoppedMonitoring'))
  }
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px' }}>
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          <div>
            <Input.TextArea
              placeholder={t('bilibili.download.urlPlaceholder')}
              value={url}
              onChange={(e) => {
            ...
            {parsing && (
               <div style={{
                 ...
                 <span>{t('bilibili.download.parsingVideoInfo')}</span>
               </div>
             )}
             {error && !parsing && (
               <div style={{
          ...
          {videoInfo && (
            <div style={{
              ...
              <Text style={{ color: '#667eea', fontWeight: 600, fontSize: '16px', display: 'block', marginBottom: '8px' }}>
                {t('bilibili.download.videoInfoParsedSuccess')}
              </Text>
              <Text style={{ color: '#ffffff', fontSize: '14px', display: 'block' }}>
                {videoInfo.title}
              </Text>
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                {getVideoType(url) === 'bilibili' ? t('bilibili.download.uploaderLabelBili') : t('bilibili.download.uploaderLabelChannel')}: {videoInfo.uploader || t('bilibili.download.unknown')} • {t('bilibili.download.durationLabel')}: {videoInfo.duration ? `${Math.floor(videoInfo.duration / 60)}:${String(Math.floor(videoInfo.duration % 60)).padStart(2, '0')}` : t('bilibili.download.unknown')}
              </Text>
            </div>
          )}
          ...
          {videoInfo && (
            <>
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>{t('bilibili.download.projectNameLabel')}</Text>
                <Input
                  placeholder={t('bilibili.download.projectNamePlaceholder')}
                  value={projectName}
              ...
              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>{t('bilibili.download.browserSelectLabel')}</Text>
                <Select
                  placeholder={t('bilibili.download.browserSelectPlaceholder')}
                  ...
                  disabled={downloading}
                >
                  <Select.Option value="firefox">Firefox (Recomendado)</Select.Option>
                ...
                <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px', marginTop: '8px', display: 'block' }}>
                  {t('bilibili.download.browserSelectHint')}
                </Text>
              </div>

              <div>
                <Text style={{ color: '#ffffff', marginBottom: '12px', display: 'block', fontSize: '16px', fontWeight: 500 }}>{t('bilibili.download.videoCategoryLabel')}</Text>
                {loadingCategories ? (
                  <Spin size="small" />
                ) : (
                  <div style={{
      ...
      {videoInfo && (
        <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'center', gap: '12px' }}>
          ...
          >
            {downloading ? t('bilibili.download.importing') : t('bilibili.download.startImport')}
          </Button>
          ...
          {downloading && (
            ...
            >
              {t('bilibili.download.stopMonitoring')}
            </Button>
          )}
        </div>
      )}
      ...
      {currentTask && (
        ...
        >
          <div style={{ marginBottom: '16px' }}>
            <Text style={{ color: '#ffffff', fontWeight: 600, fontSize: '18px' }}>{t('bilibili.download.importProgress')}</Text>
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <Text style={{ color: 'var(--ac-sub)', fontSize: '14px' }}>
                {t('bilibili.download.statusLabel')}: {
                  currentTask.status === 'completed' ? t('status.completed') :
                  currentTask.status === 'processing' ? t('status.downloading') :
                  currentTask.status === 'failed' ? t('common.failed') :
                  currentTask.status === 'pending' ? t('status.pending') :
                  currentTask.status
                }
              </Text>
          ...
          {currentTask.error_message && (
            <div style={{ 
              ...
              <Text style={{ color: '#ff4d4f', fontSize: '14px' }}>
                {t('bilibili.download.errorLabel')}: {getLocalizedProgressMessage(currentTask.error_message)}
              </Text>
            </div>
          )}
        </Card>
