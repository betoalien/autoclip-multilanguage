...
export const InlineProgressBar: React.FC<InlineProgressBarProps> = ({
  ...
  const PIPELINE_STEPS = [
    { id: 1, name: t('taskProgress.stepOutlineName') },
    { id: 2, name: t('taskProgress.stepTimingName') },
    { id: 3, name: t('taskProgress.stepScoringName') },
    { id: 4, name: t('taskProgress.stepTitleName') },
    { id: 5, name: t('taskProgress.stepClusterName') },
    { id: 6, name: t('taskProgress.stepCutName') }
  ];

  // 根据步骤ID获取步骤名称
  const getStepName = (stepId: number): string => {
    const step = PIPELINE_STEPS.find(s => s.id === stepId);
    return step ? step.name : t('taskProgress.processingEllipsis');
  };
  ...
  const [progressData, setProgressData] = useState<ProgressData>({
    ...
    stepName: currentStep > 0 ? getStepName(currentStep) : t('taskProgress.initializingEllipsis'),
    stepDetails: ''
  });
  ...
  const handleProgressUpdate = (message: any) => {
    ...
    const stepName = message.step_name || t('taskProgress.processingEllipsis');
  ...
  useEffect(() => {
    ...
    const newStepName = currentStep > 0 ? getStepName(currentStep) : t('taskProgress.initializingEllipsis');
