...
export const UnifiedStatusBar: React.FC<UnifiedStatusBarProps> = ({
  ...
  if (status === 'importing') return <ProgressRow label={t('status.importing')} percent={downloadProgress} />
  if (status === 'downloading') return <ProgressRow label={t('status.downloading')} percent={currentDownloadProgress} />

  if (status === 'processing') {
    if (!progress) return <ProgressRow label={t('status.initializingShort')} percent={0} />
    const { stage, percent, message } = progress
    if (isFailed(message)) return <StatusRow label={t('taskProgress.processFailed')} dot="var(--ac-error)" color="var(--ac-error)" />
    return <ProgressRow label={getStageDisplayName(stage)} percent={percent} />
  }

  if (status === 'completed') return <StatusRow label={t('status.completed')} dot="var(--ac-ok)" color="var(--ac-sub)" />
  if (status === 'failed') return <StatusRow label={t('taskProgress.processFailed')} dot="var(--ac-error)" color="var(--ac-error)" />

  // 等待
  return <StatusRow label={t('status.pending')} dot="var(--ac-muted)" color="var(--ac-muted)" />
}
