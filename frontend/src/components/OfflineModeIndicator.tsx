...
const OfflineModeIndicator: React.FC<OfflineModeIndicatorProps> = ({ onStatusChange }) => {
  ...
  const getStatusText = () => {
    if (!networkStatus.isOnline) {
      return t('status.offlineMode');
    }

    switch (networkStatus.connectionQuality) {
      case 'excellent':
        return t('status.networkExcellent');
      case 'good':
        return t('status.networkGood');
      case 'poor':
        return t('status.networkPoor');
      default:
        return t('status.networkUnknown');
    }
  };
  ...
  const getStatusDescription = () => {
    if (!networkStatus.isOnline) {
      return t('status.offlineDesc');
    }

    const latencyText = networkStatus.latency ? t('status.latency', { ms: networkStatus.latency }) : '';
    const lastCheck = new Date(networkStatus.lastCheck).toLocaleTimeString();

    return t('status.lastCheckWithLatency', { time: lastCheck, latency: latencyText }).trim();
  };
  ...
  return (
    <Space>
      ...
      <Button
        ...
        loading={isChecking}
        title={t('status.checkNetworkTooltip')}
        style={{
          color: '#ffffff',
