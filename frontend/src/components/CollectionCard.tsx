...
const CollectionCard: React.FC<CollectionCardProps> = ({ collection, clips, onView, onGenerateVideo, onUpdate }) => {
  ...
  return (
    <article className="ac-card">
      <div className="ac-card-thumb" onClick={() => onView(collection)} role="button" aria-label={t('collection.previewAria')}>
        {hasThumb && (
        ...
        <span className="ac-tag ac-tag--tl ac-tag--sans">
          {collection.collection_type === 'ai_recommended' ? t('collection.aiRecommended') : t('collection.manual')}
        </span>
        <span className="ac-tag ac-tag--bl">{t('collection.clipsCount', { count: collectionClips.length })}</span>
      ...
      <div className="ac-card-body">
        <div className="ac-card-title">
          <EditableCollectionTitle
            ...
            onTitleUpdate={(title) => onUpdate?.(collection.id, { collection_title: title })}
        ...
        <div className="ac-card-foot">
          <span className="meta">{t('collection.clipsCount', { count: collectionClips.length })} · {fmtDuration(totalDuration)}</span>
          <div className="ac-card-actions">
            <Btn variant="text" onClick={() => onView(collection)}>{t('collection.preview')}</Btn>
            {onGenerateVideo && <Btn variant="text" onClick={() => onGenerateVideo(collection.id)}>{t('clip.download')}</Btn>}
            <Btn variant="text" onClick={() => message.info(t('collection.uploadWip'), 3)}>{t('collection.upload')}</Btn>
          </div>
        </div>
      </div>
    </article>
