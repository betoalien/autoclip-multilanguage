...
const FeedbackDialog: React.FC<FeedbackDialogProps> = ({ open, onClose, context }) => {
  ...
  const handleSend = async () => {
    if (text.trim().length < 4) {
      message.warning(t('feedback.needMoreDetail'))
      return
    }
    setSending(true)
    try {
      ...
      if (ok) {
        message.success(t('feedback.received'))
        onClose()
      } else {
        message.info(t('feedback.analyticsDisabledUseForm'))
  ...
  return (
    <Dialog
      open={open}
      onClose={handleClose}
      title={context.source === 'failure' ? t('feedback.failureTitle') : t('feedback.title')}
      description={
        context.source === 'failure'
          ? t('feedback.failureDescription')
          : t('feedback.description')
      }
      footer={
        <>
          <div style={{ display: 'flex', gap: 4 }}>
            <Btn variant="text" size="sm" onClick={() => openExternal(FEEDBACK_FORM_URL)}>{t('feedback.formLink')} <Icon.External size={12} /></Btn>
            <Btn variant="text" size="sm" onClick={() => openExternal(FEEDBACK_ISSUES_URL)}>GitHub <Icon.External size={12} /></Btn>
          </div>
          <div className="right">
            <Btn size="sm" onClick={handleClose}>{t('common.cancel')}</Btn>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <Segmented
          size="sm"
          ariaLabel={t('feedback.categoryAriaLabel')}
          value={category}
        ...
        />
        <div className="ac-context" title={t('feedback.contextTooltip')}>
          {ctxChips.map((c) => <span key={c}>{c}</span>)}
          {!analyticsOn && <span style={{ color: 'var(--ac-warn)' }}>{t('feedback.analyticsDisabledNote')}</span>}
          {analyticsOn && surveyReady === false && <span>{t('feedback.directSubmitNote')}</span>}
        </div>
      </div>
    </Dialog>
  )
