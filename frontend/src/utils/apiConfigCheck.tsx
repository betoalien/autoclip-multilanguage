...
export const checkApiConfig = async (): Promise<ApiConfigStatus> => {
  try {
    ...
    try {
      ...
      if (!isDesktop) {
        console.log('非Desktop模式（Web模式），由后端.env管理API配置，允许创建项目')
        return {
          hasValidConfig: true,
          missingProviders: [],
          currentProvider: 'server-env'
        }
      }
      // Desktop模式下获取设置失败，返回无配置
...
export const showApiConfigModal = (missingProviders: string[], onNavigateToSettings?: () => void) => {
  const providerNames = {
    'LLM API': i18n.t('common.aiModelApi'),
    'Speech API': i18n.t('common.speechRecognitionApi')
  }

  const missingNames = missingProviders.map(p => providerNames[p as keyof typeof providerNames] || p).join('、')

  Modal.confirm({
    title: <span style={{ color: '#fff' }}>{i18n.t('common.apiConfigRequiredTitle')}</span>,
    content: (
      <div style={{ color: '#fff' }}>
        <p style={{ color: '#fff', marginBottom: '12px', fontSize: '14px' }}>{i18n.t('common.apiConfigRequiredBody')}</p>
        <p style={{ fontWeight: 'bold', color: '#40a9ff', marginBottom: '12px', fontSize: '16px' }}>{missingNames}</p>
        <p style={{ color: '#f0f0f0', fontSize: '14px' }}>{i18n.t('common.goToSettingsToConfigure')}</p>
      </div>
    ),
    okText: i18n.t('common.goToConfigure'),
...
export const getApiConfigDescription = (status: ApiConfigStatus): string => {
  if (status.hasValidConfig) {
    return i18n.t('common.apiConfiguredWithProvider', { provider: status.currentProvider })
  } else {
    return i18n.t('common.apiNotConfiguredCannotCreate')
  }
}
